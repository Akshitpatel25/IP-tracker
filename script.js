fetch(
  ...
  .then((data) => {
    ...
    document.querySelector(
      ".location_output_part"
    ).innerHTML = `${data.location["country"]}, ${data.location["city"]} <br/> ${data.location["postalCode"]}`;
    document.querySelector(
      ".timezone_output_part"
    ).innerHTML = `${data.location["timezone"]}`;
  ...
  if (ipv4Pattern.test(ip)) {
    ...
    fetch(ipaddress)
      ...
      .then((data) => {
        ...
        // Convert the object to a JSON string and store it in local storage
        localStorage.setItem("newlocation", JSON.stringify(newlocationData));
